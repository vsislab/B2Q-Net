...
class SCALayer(nn.Module):
    ...
    def __init__(self, action_dim, frame_dim, nhead, dim_feedforward=2048, dropout=0.1, attn_dropout=0.1,
        ...
        self.ca_qpos_sine_proj = nn.Linear(action_dim, action_dim)
    ...
    def forward(self, tgt, memory,
        ...
        # query=add_positional_encoding(tgt, query_sine_embed)
        query=add_positional_encoding(tgt, query_pos)
        value = memory
...
class SCADecoder(nn.Module):
    ...
    def forward(self, tgt, memory, pos: Optional[Tensor] = None, query_pos: Optional[Tensor] = None, query_sine_embed: Optional[Tensor] = None):
        ...
        for layer in self.layers:
            output = layer(output, memory, pos=pos, query_pos=query_pos, query_sine_embed=query_sine_embed)
